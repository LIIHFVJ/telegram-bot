...
# --- إعدادات Supabase ---
SUPABASE_URL = "https://snvickveazzkusfnytfn.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNudmlja3ZlYXp6a3VzZm55dGZuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjgyNTUwMjI2fQ.LRfr8hwYC-tgOIJq_8TxqhSyD-YL3ITDSMMlyjNWsHw"
HEADERS = {
    "apikey": SUPABASE_KEY,
...
def supabase_insert(table, data):
    try:
        ...
        print(f"❌ Error in supabase_insert: {e}")
        return None
...
def supabase_select(table, filters=None):
    try:
        ...
        print(f"❌ Error in supabase_select: {e}")
        return []
...
def supabase_update(table, data, filters):
    try:
        ...
        print(f"❌ Error in supabase_update: {e}")
        return None
...
def supabase_delete(table, filters):
    try:
        ...
        print(f"❌ Error in supabase_delete: {e}")
        return 500
...
def callback_inline(call):
    ...
    if call.data == "menu_gen":
        ...
        total = len(rows)
        used = len([r for r in rows if isinstance(r, dict) and r.get('is_used')])
        ...
        for r in rows:
            ...
            file_content += f"Code: {r.get('code', 'N/A')} | Dur: {r.get('duration', 'N/A')} | Status: {status}\n"
...
def process_extend_step(message):
    target_code = message.text.strip()
    rows = supabase_select("codes", {"code": f"eq.{target_code}"})
    if not rows or not isinstance(rows[0], dict) or not rows[0].get('expiry_date'):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"❌ خطأ أثناء التمديد: {e}", reply_markup=main_menu())
    ...
    if rows:
        ...
        for r in rows:
            if isinstance(r, dict):
                text += f"🔑 `{r.get('code', '---')}` ({r.get('duration', '---')})\n"
